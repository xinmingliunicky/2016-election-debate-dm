...
#head(users)
tweets <- read.csv("user_setA/tweets_debateVP.csv", stringsAsFactors = FALSE)
#head(tweets)
...
ggplot(vis.tfidf, aes(x = x, y = y, color=party)) +
  geom_point() + geom_text(aes(x = x, y = y - 0.2, label = id)) + scale_color_manual(values = c("#146EE3","#EB3434")) + ggtitle("VP Debate PA clustering based on TF-IDF") +
...
ggplot(vis.lsa, aes(x = x, y = y, color=party)) +
  geom_point() + geom_text(aes(x = x, y = y - 0.2, label = id)) + scale_color_manual(values = c("#146EE3","#EB3434")) + ggtitle("VP Debate PA clustering based on LSA") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
ggplot(vis.nmf, aes(x = x, y = y, color=party)) +
  geom_point() + geom_text(aes(x = x, y = y - 0.2, label = id)) + scale_color_manual(values = c("#146EE3","#EB3434")) + ggtitle("VP Debate PA clustering based on NMF") +
  theme(plot.title = element_text(hjust = 0.5))
```
